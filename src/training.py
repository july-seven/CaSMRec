...
import torch
import numpy as np
from util import llprint, multi_label_metric, ddi_rate_score, get_n_params,final_ddi
import time
...
def eval_one_epoch(model, data_eval, voc_size, drug_data):
    ...
    ddi_rate = ddi_rate_score(smm_record, path='../data/mimic4/output/ddi_A_final.pkl')
...
def Test(model, model_path, device, data_test, voc_size, drug_data):
    with open(model_path, 'rb') as Fin:
        model.load_state_dict(torch.load(Fin, map_location=device),strict=False)
    ...
    for _ in range(10):
       # test_sample = np.random.choice(data_test, sample_size, replace=True)
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = eval_one_epoch(model, test_sample, voc_size, drug_data)
...
def Train(
    ...
    optimizer, log_dir, coef, target_ddi, ddi_adj,ehr_train_pair,EPOCH=50
):
    ...
    for epoch in range(EPOCH):
        ...
        for step, input_seq in enumerate(data_train):
            for adm_idx, adm in enumerate(input_seq):
                ...
                current_ddi_rate = ddi_rate_score(
                    [[y_label]], path='../../hehomole/data/mimic4/output/ddi_A_final.pkl'
                )
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = \
            eval_one_epoch(model, data_eval, voc_size, drug_data,ddi_adj,ehr_train_pair)
        ...
        with open(os.path.join(log_dir, 'best.txt'), 'a') as Fout:
            Fout.write(f'{best_epoch}\t{best_ja}\t{ddi_rate}\n')
